...
function buildSiteLinks(site: Site): SiteLink[] {
  const links: SiteLink[] = [];
  if (site.whatsappGroupLink) {
    links.push({
      key: "group",
      label: "Join WhatsApp Group",
      subtitle: "Community & support",
      url: site.whatsappGroupLink,
      icon: Users,
    });
  }
  if (site.channelLink) {
  ...
  if (site.repoUrl) {
    links.push({
      key: "repo",
      label: "View Source",
      subtitle: site.repoUrl.replace("https://github.com/", ""),
      url: site.repoUrl,
      icon: Github,
    });
  }
        ...
        <a key={link.key} href={link.url} target="_blank" rel="noopener noreferrer">
          ...
          >
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-[#00ff00]/10 border border-[#00ff00]/15 flex items-center justify-center shrink-0">
              <link.icon className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: "#22c55e" }} />
            </div>
...
export default function Home({ site }: { site: Site }) {
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-3 sm:px-6 py-4 sm:py-6">
        <nav className="app-topbar sticky top-3 z-20 flex items-center justify-between gap-3 rounded-xl px-3 sm:px-5 py-2.5 mb-10 sm:mb-16">
          <Link href="/" className="flex items-center gap-2.5 min-w-0">
            <Bot className="w-4 h-4 shrink-0" style={{ color: "#22c55e" }} />
        ...
        <header className="text-center mb-8 sm:mb-14">
          ...
          <p className="text-gray-500 font-mono text-sm max-w-md mx-auto leading-relaxed">
            Link your device to {site.name} via WhatsApp
          </p>
          ...
          <div className="md:col-span-3 space-y-6">
            <GlassCard className="p-4 sm:p-6 md:p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-base sm:text-lg font-bold text-white font-mono">{site.name} Pair</h2>
